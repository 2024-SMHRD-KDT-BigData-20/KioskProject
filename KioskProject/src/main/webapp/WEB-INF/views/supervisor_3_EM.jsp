...
<script type="text/javascript"
	src="<c:url value='/resources/js/supervisor_EM.js' />"></script>
<script type="text/javascript">
	var cpath = "${cpath}";
</script>
</head>
<body>
	...
	<div class="container3">
		<div class="search">
			<form class="search_container" action="${cpath }/emp_search.do"
				method="get">
				<input type="text" id="search_bar" placeholder="이름 검색"> <img
					class="search_icon"
				...
				<tbody>
					<c:forEach items="${emp_list }" var="m">
						<tr>
							...
							<td class="emp_auth"><c:choose>
									<c:when test="${m.emp_auth == 0}">
                            상위 관리자
                        </c:when>
									<c:when test="${m.emp_auth == 1}">
                            하위 관리자
                        </c:when>
									<c:otherwise>
                            알 수 없음
